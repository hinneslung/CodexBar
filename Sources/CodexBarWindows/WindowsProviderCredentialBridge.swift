...
struct WindowsProviderCredentialBridge: Sendable {
    ...
    enum BridgeError: LocalizedError, Equatable, Sendable {
        ...
        var errorDescription: String? {
            ...
            case let .expiredCredential(provider):
    ...
    func environmentOverrides(
        ...
        base: [String: String]) throws -> [String: String]
    {
    ...
    private static func secret(from entry: AuthEntry, for candidate: Candidate) throws -> String? {
        ...
        case .oauthAccess:
            ...
            if let expires = entry.expires, expires > 0, expires <= currentTimeMilliseconds {
    ...
    private static func loadSelectedWSLAuthData(environment: [String: String]) throws -> Data? {
        guard
            let rawDirectory = environmentValue(
                named: wslOpenCodeDataHomeEnvironmentKey,
                in: environment),
            let directory = nonEmpty(rawDirectory)
        else { return nil }
