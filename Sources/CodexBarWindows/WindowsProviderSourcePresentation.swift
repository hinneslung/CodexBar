...
struct WindowsProviderSourcePresentation: Equatable, Sendable {
    ...
    var formattedValue: String {
        if case let .compatibility(value) = self.kind {
    ...
    static func configuredFallback(
        ...
        credentialLabel: String? = nil) -> Self
    {
        let distribution =
    ...
    private var sourceLabel: String {
        ...
        case .openCode:
            "OpenCode"
        case let .manual(label), let .upstream(label):
            Self.upstreamLabel(label)
        case let .compatibility(label):
            Self.upstreamLabel(label)
        }
    }
    ...
    private static func upstreamLabel(
        _ value: String?,
        provider: WindowsProviderID? = nil) -> String
    {
    ...
    private static func automaticUpstreamLabel(
        _ value: String?,
        provider: WindowsProviderID?) -> String
    {
        if let provider,
           let source = normalized(value)?.lowercased(),
           let label = automaticSourceLabelOverrides[provider]?[source]
        {
            return label
        }
    ...
    static func summary(provider: WindowsProviderID) -> String {
        ...
        return self.labels(
            ...
            manualLabels: WindowsProviderConfigurationCatalog.byProvider[provider]?
                .manualCredentialSets.map(\.label) ?? []).joined(separator: " · ")
    }

    static func labels(
        providerAppOrCLI: Bool,
        supportsOpenCode: Bool,
        manualLabels: [String]) -> [String]
    {
        var labels: [String] = []
