...
enum WindowsHiddenProcessRunner {
    ...
    static func run(
        ...
        standardInput: Data? = nil) throws -> WindowsHiddenProcessResult
    {
        ...
        let attributeList = try Self.inheritedHandleList(
            handles: [childInput, outputHandles.child, errorHandles.child].compactMap(\.self))
    ...
    static func environmentBlock(
        base: [String: String],
        overrides: [String: String]) throws -> [WCHAR]
    {
    ...
    private static func openOutput(
        security: inout SECURITY_ATTRIBUTES) throws -> (child: HANDLE?, parent: HANDLE?)
    {
    ...
    {
        guard data != nil else {
            guard let input = openNullInput(security: &security) else {
                throw WindowsCanonicalCLIError.launchFailed
            }
            return (input, nil)
        }
    ...
    private static func stop(process: HANDLE, workers: [WindowsPipeWorker]) {
        ...
        _ = WaitForSingleObject(process, 1000)
        for worker in workers {
            _ = WaitForSingleObject(worker.thread, 1000)
        }
    }
    ...
    private static func inheritedHandleList(
        handles: [HANDLE]) throws -> WindowsProcessAttributeList
    {
