...
struct WindowsProviderConfigurationField: Sendable, Equatable {
    ...
    func normalized(_ value: String) -> String? {
        ...
        case let .browserCredential(policy):
    ...
    func displaySafeValidationResult(_ value: String) -> WindowsBrowserCredentialValidationResult? {
        guard case let .browserCredential(policy) = self.validation else { return nil }
...
struct WindowsProviderCredentialSet: Sendable, Equatable {
    ...
    var inputPolicy: WindowsBrowserCredentialPolicy? {
        for field in self.fields {
            if case let .browserCredential(policy) = field.validation { return policy }
        }
        return nil
    }
}
...
enum WindowsProviderConfigurationCatalog {
    ...
    private static func browserSet(
        ...
        executionMode: WindowsCanonicalCLIInvocation.ExecutionMode = .usage) -> WindowsProviderCredentialSet
    {
    ...
    private static func browserField(
        ...
        exactPath: String? = nil,
        allowsQuery: Bool = true,
        cookieRequired: Bool = true) -> WindowsProviderConfigurationField
    {
        ...
        case let .fullRequest(allowCookieHeader):
    ...
    private static func apiSchema(
        ...
        secretTransport: WindowsProviderCredentialSet
            .SecretTransport = .stagedConfig) -> WindowsProviderConfigurationSchema
    {
    ...
    private static func webSchema(
        _ provider: WindowsProviderID,
        _ cliName: String,
        set: WindowsProviderCredentialSet) -> WindowsProviderConfigurationSchema
    {
        WindowsProviderConfigurationSchema(
            provider: provider,
            cliName: cliName,
...
enum WindowsProviderConfigurationValidator {
    static func accepts(
        _ rawValue: String,
        validation: WindowsProviderConfigurationField.Validation) -> Bool
    {
        ...
        case let .allowedPrefixes(prefixes, minimumSuffixLength, _):
            return !value.contains(where: \.isNewline)
    ...
    static func rejectionMessage(
        _ rawValue: String,
        validation: WindowsProviderConfigurationField.Validation) -> String?
    {
        guard !self.accepts(rawValue, validation: validation) else { return nil }
        guard case let .allowedPrefixes(_, _, message) = validation else { return nil }
        return message
    }
    ...
    private static func validatedURL(
        _ raw: String,
        allowingHTTPFor permitsHTTP: (String) -> Bool) -> URL?
    {
