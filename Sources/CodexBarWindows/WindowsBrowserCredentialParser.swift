...
enum WindowsBrowserCredentialParser {
    ...
    static func validate(
        _ rawValue: String,
        policy: WindowsBrowserCredentialPolicy) -> WindowsBrowserCredentialValidationResult
    {
        ...
        case let .opaqueSession(defaultCookieName, recognizedCookieNames):
    ...
    private static func validateCookieInput(
        ...
        acceptsCURL: Bool) -> WindowsBrowserCredentialValidationResult
    {
        var rawCookie = value
        ...
        if Self.looksLikeCURL(value) {
            ...
            case let .success(parsed): capture = parsed
            case let .failure(message): return .invalid(message)
            }
    ...
    private static func validateOpaqueSession(
        ...
        recognizedCookieNames: [String]) -> WindowsBrowserCredentialValidationResult
    {
        ...
        guard !value.contains(";"), !value.contains(where: { $0.isWhitespace || $0.isNewline }),
              !value.contains(
                  where: { $0.isASCII && $0.asciiValue.map { $0 < 0x21 || $0 == 0x7F } == true })
    ...
    private static func hasRequiredCookies(
        ...
        policy: WindowsBrowserCredentialPolicy) -> Bool
    {
    ...
    private static func validatedHost(
        _ url: URL,
        policy: WindowsBrowserCredentialPolicy) -> String?
    {
