...
"$verify" success success false skipped false true success success success >/dev/null
"$verify" success success false skipped false false skipped success success >/dev/null
...
assert_verify_fails success success false skipped true true success success success
assert_verify_fails success success true success true true success success success
assert_verify_fails success success false success false true success success success
...
assert_verify_fails success success true success false true success success failure
...
for macos_required in true false; do
  ...
  for musl_required in true false; do
    ...
    for linux_result in failure cancelled skipped '' unknown; do
      assert_linux_verify_fails "${linux_result:-<empty>}" "${valid_args[@]}" "$linux_result" success
    done
    ...
    for failed_result in failure cancelled; do
      assert_verify_fails "$failed_result" success "$macos_required" "$macos_result" \
        false "$musl_required" "$musl_result" success success
      assert_verify_fails success "$failed_result" "$macos_required" "$macos_result" \
        false "$musl_required" "$musl_result" success success
      if [[ "$macos_required" == true ]]; then
        assert_verify_fails success success true "$failed_result" false "$musl_required" "$musl_result" \
          success success
      fi
...
if needs is None:
    sys.exit("missing aggregate needs")
need_lines = needs.group(1).splitlines()
if "      - build-linux-cli" not in need_lines:
    sys.exit("lint-build-test must need build-linux-cli")
if "      - swift-test-windows" not in need_lines:
    sys.exit("lint-build-test must need swift-test-windows")
...
if arguments[9] != "${{ needs.swift-test-windows.result }}":
    sys.exit("aggregate verifier argument nine must be needs.swift-test-windows.result")
PY
