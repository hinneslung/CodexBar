...
struct WindowsBundledWSLCLIProvisionerTests {
    @Test
    func `valid release payload is accepted without following links`() throws {
    ...
    @Test
    func `unsafe release version cannot become a WSL path`() throws {
    ...
    @Test
    func `install plan uses direct fixed commands in an application owned directory`() {
    ...
    @Test
    func `translated WSL paths are bounded and absolute`() {
        #expect(
    ...
    @Test
    func `payload location is adjacent to the packaged executable`() {
    ...
    @Test
    func `same version payload with a mismatched CLI checksum is never returned`() throws {
        let directory = try Self.payloadDirectory(version: "0.54.1")
        defer { try? FileManager.default.removeItem(at: directory) }
    ...
    @Test
    func `same version stale launcher is replaced from the current payload`() throws {
        let directory = try Self.payloadDirectory(version: "0.54.1")
        defer { try? FileManager.default.removeItem(at: directory) }
    ...
    @Test
    func `bundled payload can be provisioned and executed in a live WSL distribution`() throws {
    ...
    private static func payloadDirectory(
        version: String,
        executableContents: String = "executable") throws -> URL
    {
