...
struct WindowsProviderSnapshotPublisherTests {
    @Test
    func `credential mutation during a delayed sibling rejects the stale provider at commit`() async throws {
        ...
        let outcome = try await WindowsProviderSnapshotPublisher.publish(batch.value) { snapshot in
    ...
    @Test
    func `publication rejects provider mutex contention without blocking the UI thread`() async throws {
