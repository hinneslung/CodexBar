...
struct WindowsFinalCredentialAlignmentTests {
    ...
    @Test
    func `catalog method counts and aligned provider copy are exact`() throws {
    ...
    @Test
    func `MiniMax accepts only nonempty Coding Plan keys with pinned safe guidance`() throws {
        let set = try #require(
    ...
    @Test
    func `DeepSeek stages one ephemeral token account and no API key`() throws {
        ...
        #expect(try Foundation.UUID(uuidString: #require(account["id"] as? String)) != nil)
