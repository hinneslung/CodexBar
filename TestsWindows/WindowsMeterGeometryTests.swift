  ...
  struct WindowsMeterGeometryTests {
    ...
    func `spinner size follows DPI once and stays centered inside its hit target`(dpi: Int32) {
      func scaled(_ value: Int32) -> Int32 { Int32((Double(value * dpi) / 96).rounded()) }
      for logicalDiameter: Int32 in [14, 18] {
        ...
        #expect(dots.count == 8)
        let left = dots.map(\.left).min()!
        let right = dots.map(\.right).max()!
        let top = dots.map(\.top).min()!
        let bottom = dots.map(\.bottom).max()!
